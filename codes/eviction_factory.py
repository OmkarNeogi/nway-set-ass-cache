...
The Smallest first algorithm can serve as an example - it has it's own (very different from linked list) data structure 
and different style of operations (that are done on the data structure - push, peak and pop). 
"""
...
def eviction_factory(eviction_strategy):
    ...
    To use a custom algorithm, add it to the dictionary mentioned here (eviction_factory.py)

    Raises ValueError if the eviction_strategy is not recognised in the EVICTION_STRATEGY_REGISTRY.
